...
- 관리자 페이지는 새로고침 시와 1초 주기로 `/api/admin/sync/status`를 조회해 현재 상태를 표시합니다.
...
- 관리자 페이지 우측에는 전체 API 호출 로그 패널이 표시되고, 일정 주기로 자동 갱신됩니다.
- 관리자 페이지 우측 로그 패널은 15초 주기로 갱신됩니다.
- 로그 패널의 `로그 비우기` 버튼은 현재 DB 로그를 `logs/log_history.csv`와 `logs/log_history_yyyyMMdd_HHmmss.csv`로 백업한 뒤, 관리자 페이지와 DB의 호출 로그를 비웁니다.
- 관리자 페이지 우측 패널은 가독성을 위해 기본 폭을 조금 넓게 배치합니다.
