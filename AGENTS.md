# AGENTS.md

## 문서 규칙

- `README.md`와 `AGENTS.md`는 항상 한국어로 작성합니다.
- 기능이나 구조가 바뀌면 `README.md`와 `AGENTS.md`를 함께 갱신합니다.
- 저장 시 UTF-8 인코딩을 유지해 한글이 깨지지 않게 합니다.

## Git 규칙

- 소스 코드, 설정, 마이그레이션, 문서, 실행에 필요한 보조 스크립트는 Git에 포함합니다.
- 로그, 빌드 산출물, 로컬 비밀정보, IDE 전용 파일은 Git에 포함하지 않습니다.
- 문서성 클레임 기록은 `docs/`에 두되, 오래된 기록은 `docs/archive/`로 옮깁니다.
- 제한 동기화 기록은 루트가 아니라 `logs/` 폴더 아래에 저장합니다.

## 프로젝트 역할

- 이 프로젝트는 CheapestOilFinder Android 앱을 위한 Spring Boot 백엔드입니다.
- Android 앱은 오피넷 API를 직접 호출하지 않습니다.
- 오피넷 데이터 수집과 정규화는 이 백엔드가 담당합니다.
- 주유소 검색, 경로 주변 검색, 추천 계산용 데이터 준비는 서버가 담당합니다.
- 앱은 백엔드 API만 호출하고, 백엔드는 PostgreSQL/PostGIS를 조회하거나 갱신합니다.

## 실행 전제

- 로컬 DB는 Docker로 실행합니다.
- Spring 서버는 `.\gradlew.bat bootRun`으로 실행 가능한 상태를 유지합니다.
- 개발 흐름이 바뀌면 README에도 동일한 실행 방법을 반영합니다.

## 오피넷 및 비밀정보

- 오피넷 API 키는 백엔드 프로젝트 루트의 `secrets/opinet-api-key.txt` 또는 환경변수로만 제공합니다.
- 오피넷 주유소 동기화는 `uni_id` 기본키를 기준으로 PostgreSQL 인덱스와 `ON CONFLICT` 업서트를 사용하며, 메모리 순차 탐색을 하지 않습니다.
- `secrets/` 폴더는 Git, Docker 이미지, Android 릴리즈 패키지에 포함되면 안 됩니다.
- API 키를 코드, 테스트 fixture, README 예시 실제값, 로그에 직접 적지 않습니다.

## 섹터 동기화 정책

- `sync_sector`, `sector_sync_log` 기반 섹터 동기화 구조를 유지합니다.
- 섹터 등급은 `HOT`, `WARM`, `COLD`, `DISABLED` 구조를 유지합니다.
- 오피넷 무료 API 호출 제한을 항상 고려합니다.
- 오피넷 연동은 기본적으로 활성화되어 있으며, 필요 시 `OPINET_ENABLED=false`로 끌 수 있습니다.
- `sync_sector.katec_x`, `sync_sector.katec_y`, `sync_sector.radius_m` 값을 기준으로 `aroundAll.do`를 호출합니다.
- 자동 동기화는 `HOT` 하루 1회, `WARM` 3일 1회, `COLD` 수동 요청만 허용하는 방식으로 유지합니다.
- 섹터 단위 호출은 순차 처리하고, 다음 섹터로 넘어가기 전에 기본 30초 간격을 둡니다.
- 한 섹터 내부의 유종 호출도 순차 처리하고, 각 유종 호출 사이에 기본 30초 간격을 둡니다.
- 1차 호출에서 실패한 유종은 실패 큐에 넣고, 모든 1차 호출이 끝난 뒤에 1회만 재시도합니다.
- 재시도까지 실패하면 더는 호출하지 않습니다.
- 주유소와 유가 데이터의 최신 시각은 업서트 시점의 `updated_at`로 판단할 수 있어야 합니다.
- 관리자 페이지의 강제 재호출 버튼은 `HOT_ONLY`, `HOT_WARM`, `WARM_ONLY`, `COLD_ONLY` 스코프와 1:1로 맞춥니다.
- 강제 재호출이 실행 중일 때는 상단 상태 표시를 프론트 타이머로 계산하지 않고, 백엔드가 내려주는 현재 작업 정보와 경과시간으로 보여줍니다.
- 관리자 페이지는 새로고침 시와 1초 주기로 백엔드 상태 API를 폴링합니다.
- 수동 재호출이 이미 실행 중이면 같은 스코프의 요청은 waiting queue에 추가하고, 같은 버튼을 중복으로 눌러도 이미 대기열에 있으면 중복 enqueue하지 않습니다.

## 관리자 섹터 맵

- `/admin/sectors.html`은 `sync_sector` 상태를 시각적으로 보여주는 정적 관리자 페이지입니다.
- 지도는 지명 포함 대한민국 백지도 위에 반투명 허니콤 섹터를 올리는 방식입니다.
- 백지도와 허니콤은 같은 캔버스에서 함께 움직여야 합니다.
- 관리자 페이지는 본문 전체가 스크롤되지 않도록 하고, 우측 사이드패널만 내부 스크롤되게 유지합니다.
- 휠 확대/축소와 우하단 `+ / -` 버튼은 유지하되, 확대/축소 기준점은 지도 중앙으로 둡니다.
- 상태 버튼으로 `HOT`, `WARM`, `COLD`, `DISABLED`를 변경할 수 있어야 합니다.
- `DISABLED`는 표시 상태이며, `sync_tier`는 유지하고 `enabled`만 끕니다.
- `전체 활성화`, `전체 비활성화`는 소수 섹터 테스트를 위한 임시 운영 기능입니다.
- `HOT만 재호출`, `HOT+WARM 재호출`, `WARM만 재호출`, `COLD만 재호출` 버튼은 각각 관리자가 즉시 오피넷 재호출을 요청하는 수동 운영 기능이며, 유종 체크박스로 선택한 항목만 함께 호출합니다.
- 섹터 시각화 관련 변경이 생기면 README의 관리자 섹터 맵 섹션도 함께 갱신합니다.

## 5km 커스텀 헥사곤

- H3는 사용하지 않습니다.
- 대한민국 GeoJSON과 자체 5km 헥사곤 생성 로직을 사용합니다.
- `/api/admin/sectors/generate/custom-hex?sideLengthMeters=5000`는 대한민국 폴리곤 내부 중심점 기준으로 헥사곤 섹터를 생성하거나 갱신합니다.
- 각 섹터는 WGS84 중심 좌표, KATEC 중심 좌표, PostGIS Polygon, tier, enabled 상태를 함께 저장합니다.
- 오피넷 호출은 섹터 중심 좌표와 반경을 기준으로 수행합니다.

## 백엔드 계약

- 주유소 검색 API는 프론트엔드 가이드와 맞는 상태를 유지합니다.
- 계약이 바뀌면 README와 이 파일을 함께 갱신합니다.
- 프론트엔드가 이해해야 하는 요청/응답 예시는 `docs/FRONTEND_BACKEND_API_GUIDE.md`에 유지합니다.

## 관리자 로그 패널과 유종 선택

- 관리자의 수동 강제 재호출은 일반 휘발유, 고급 휘발유, 경유 체크박스로 유종을 고른 뒤 실행합니다.
- 선택된 유종만 `fuelTypes` 파라미터로 전송하며, 선택되지 않은 유종은 호출하지 않습니다.
- 관리자 페이지는 최근 오피넷 호출 로그를 보여주는 패널을 포함하며, `GET /api/admin/sync/logs/recent`를 사용합니다.
## 오피넷 호출 우선순위 정책

- 오피넷 재호출은 `FRONTEND` > `MANUAL` > `AUTO` 순서로 처리합니다.
- 자동 호출, 수동 재호출, 프론트엔드 재호출은 서로 다른 큐로 취급하되, 현재 진행 중인 호출이 끝나는 시점마다 우선순위가 가장 높은 큐를 다시 선택합니다.
- 같은 `source`와 같은 요청 조건이 이미 대기열에 있으면 중복 enqueue하지 않습니다.
- 큐가 바뀌어도 오피넷 호출 간 기본 30초 간격은 반드시 유지합니다.
- 관리자 페이지의 수동 재호출은 유종 체크박스와 최근 호출 로그 패널을 함께 사용합니다.
- 관리자 페이지의 호출 로그 패널은 백엔드의 전체 호출 로그를 읽어오며, `로그 비우기` 버튼은 현재 로그를 `logs/log_history.csv`와 타임스탬프 파일로 백업한 뒤 DB와 화면에서 비웁니다.
- 관리자 페이지의 호출 로그 패널은 15초 주기로 갱신합니다.
  - 관리자 페이지 우측 패널은 약간 넓게 보여서 상태, 로그, 수동 호출 버튼이 덜 답답하게 배치됩니다.
  - ## 좌표 변환 메모

- 오피넷 KATEC 좌표는 단순 투영 역산만 하지 않고, Bessel datum과 WGS84 사이의 보정까지 포함해서 다룹니다.
- 좌표 변환 결과를 확인할 때는 WGS84 roundtrip과 legacy projection-only 결과의 차이를 함께 본 뒤 판단합니다.
