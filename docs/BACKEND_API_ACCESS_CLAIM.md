...
- `POST /api/stations/route` 는 이제 네이버 Directions 5 경로 응답을 함께 내려줄 수 있습니다.
- 응답의 `route` 필드에는 경로 polyline 과 거리/시간/통행료/유류비 요약이 포함됩니다.
- 네이버 키는 `secrets/naver_maps_client.txt` 에 두 줄로 저장합니다.
